...
namespace Bank.WebUI
{
    public class Startup
    {
        ...
        private async Task CreateUserRoles(IServiceProvider serviceProvider)
        {
            ...
            var IdentityUser = new IdentityUser() { Email = Constants.Email, UserName = Constants.UserName };
            if (!await UserManager.Users.AnyAsync(x => x.Email == IdentityUser.Email))
            {
                var result = await UserManager.CreateAsync(IdentityUser, Constants.AdminPassword);
                if (result.Succeeded)
                {
                   // await UserManager.AddClaimAsync(IdentityUser, new Claim("sub", IdentityUser.Id));
                    await UserManager.AddClaimAsync(IdentityUser, new Claim("admin", IdentityUser.Id));
                    var roleCheck = await RoleManager.RoleExistsAsync(Constants.Admin_Role);
                    if (!roleCheck)
                    {
                        //create the admin roles and seed them to the database  
                        await RoleManager.CreateAsync(new IdentityRole(Constants.Admin_Role));
                    }
                    await UserManager.AddToRoleAsync(IdentityUser, Constants.Admin_Role);
                    var userRoleCheck = await RoleManager.RoleExistsAsync(Constants.User_Role);
                    if (!userRoleCheck)
                    {
                        //create the roles and seed them to the database  
                        await RoleManager.CreateAsync(new IdentityRole(Constants.User_Role));
                    }
                    //asign user role to admin
                    await UserManager.AddToRoleAsync(IdentityUser, Constants.User_Role);
                }
